...
const executionConsistencyPrompt = "" +
	...
	"涉及 codex-local 的本地项目开发/分析任务时，必须先基于已注入 Projects Index 选择项目；提交时 metadata 至少提供 working_dir（且必须位于 APP_PROJECT_ROOT_DIR 下），或提供 project_id 让系统派生 working_dir；新项目还需显式给 create_project=true。\n" +
