---
name: "A2A 任务编排"
description: "当用户要求调用已接入外部 Agent 完成任务，尤其是大型开发、仓库分析或多步产出任务时使用"
---

目标：基于已接入 A2A Agent 完成任务编排与结果回读。
定位：数字分身默认负责调度、状态回读与结果汇总；匹配的 A2A Agent 负责主体执行。
...
规则：大型开发、仓库分析、多步产出类任务，若存在匹配的已启用 A2A Agent，优先委托该 Agent 执行主体工作，禁止数字分身先用本地 bash 展开主体开发。
