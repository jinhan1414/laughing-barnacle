...
- [x] 2.1 新增内置工具 `context__read`（只读）并接入 Agent 工具注册与参数校验。
- [x] 2.2 新增内置工具 `maintenance__write`（只写）并复用现有 `/api/*` service 执行写入。
- [x] 2.3 重构 `internal/agentprompt/defaults.go`，将系统提示改为固定 Heading 分块并移除重复回答策略。
- [x] 2.4 调整 `internal/agent/reply_generation.go`，确保“回答策略”只保留单一注入来源且段落顺序稳定。
- [x] 2.5 重构 `internal/agent/tool_runtime_prompt.go`，统一环境警戒、API 路由约束、执行证据硬规则与原生工具优先级。
- [x] 2.6 同步 `internal/skills/store.go` 中维护类 Skill 文案，移除本地 API 读写的命令行说明，强制改为原生工具调用。
...
- [x] 3.6 单测：验证提示词中不存在“本地 API 可用 shell 命令写入”的文案。
- [x] 3.7 回归 `go test ./...`（后端单测总超时 60 秒内）。
