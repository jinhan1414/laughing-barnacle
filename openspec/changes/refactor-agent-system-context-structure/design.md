...
- Decision: 提示词只保留编排信息，参数校验职责下沉到工具契约
  - 在 runtime prompt 中保留“何时调用哪个工具”的路由约束。
  - API 参数结构、必填字段、格式校验由 Tool Schema + 服务端统一校验。
  - 原因：降低 system prompt 负载，减少长文本参数细节导致的注意力稀释。

- Decision: 执行约束改为正向表达
  - 使用“先调用工具再回复”“无结果时明确未执行与阻塞原因”等正向动作指令。
  - 减少高密度负面禁令文案，保留必要安全边界。
  - 原因：降低负面词触发的注意力偏置与保守拒答倾向。
