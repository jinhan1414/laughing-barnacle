# Change: 简化 linux__bash 为命令直传

## Why
当前 `linux__bash` 的调用参数是对象（`{"command":"...","timeout_sec":...}`），在真实对话中会产生多层转义与冗余键名，导致模型更容易生成不稳定参数。  
用户期望改为“只写完整命令”（如 `curl ...`），降低调用复杂度与 token 开销。

## What Changes
- 将 `linux__bash` 的参数契约改为“单一命令字符串”，不再要求 `command` 键包装。
...
- Affected code:
  ...
  - `data/skills/*.md`（仅涉及 linux__bash 参数约束文案）
